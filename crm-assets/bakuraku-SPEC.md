# SPEC.md — bakuraku (Lark-native バクラク代替システム)

## コンセプト

**バクラク（LayerX）を使わずに、Lark（飛書）だけでバクラク相当のバックオフィス機能を実現する。**
...
- `bakuraku` はこれをオーケストレートするラッパーとして実装

### bakuraku サーバー（Express）
...
[申請者]
  ...
  │     ↓ Automation → HTTP Request → bakuraku サーバー
  │
  ├─ bakuraku サーバー（Express / Node.js）
...
ユーザー → Lark Bot に領収書画像を送信
  ↓ im.message.receive_v1 Webhook → bakuraku サーバー POST /ocr
...
Base Automation（フォーム送信トリガー）
  ↓ HTTP Request → bakuraku サーバー POST /approval-create
...
```
Base（承認済みレコード更新）
  ↓ Base Automation → bakuraku サーバー POST /freee-sync
...
```
bakuraku サーバー → POST https://expense.moneyforward.com/api/external/v1/expense_reports
```

---

## bakuraku setup の自動化範囲

`bakuraku setup` コマンドで自動化できる範囲（CLI で完結するもの）：
...
2. Base Automation（+workflow-create 設定済み）
   → bakuraku サーバー POST /approval-create に Webhook POST

3. bakuraku サーバーが Approval インスタンス作成
   POST /approval/v4/instances → instance_code を Base に書き込み

4. bakuraku サーバーが承認者へ Interactive Card 送信
   （承認する / 却下する ボタン付き）
...
7. 月次バッチ（bakuraku report monthly）
   → Base データを集計してレポートカードをチャンネルに投稿
   → （オプション）bakuraku サーバー経由で freee に仕訳作成
```
...
- [ ] bakuraku サーバーデプロイ（Cloud Functions 等）+ Lark Developer Console に Callback URL 登録
- [ ] `lark-cli config init` + `lark-cli auth login`

### Phase 1 — 基盤構築（CLI 一括実行）
- [ ] `bakuraku setup`: Base + テーブル + フィールド作成
- [ ] `bakuraku setup`: フォーム + 質問作成
- [ ] `bakuraku setup`: 通知用ワークフロー作成・有効化
- [ ] `bakuraku setup`: ダッシュボード + グラフ作成（直列）
- [ ] `pnpm dev` → `[auth] tenant_access_token acquired` を確認

### Phase 2 — 承認フロー
- [ ] bakuraku サーバー `POST /approval-create` 実装済み（`src/server/index.ts`）
...
### Phase 3 — OCR・領収書処理
- [ ] bakuraku サーバー `POST /ocr` 実装済み（`src/server/index.ts`）
- [ ] Bot 有効化 + `im.message.receive_v1` を `/ocr` に登録

### Phase 4 — 会計ソフト連携
- [ ] `bakuraku auth freee` で OAuth トークンを取得・保存
- [ ] `POST /freee-sync` 実装済み（`src/server/index.ts` + `src/lark/freee.ts`）
- [ ] Base Automation → `/freee-sync` 呼び出し設定（承認済トリガー）
- [ ] （オプション）MoneyForward 連携

### Phase 5 — レポート・分析
- [ ] `bakuraku report monthly`: Base 集計 → Bot チャンネル投稿
- [ ] Base ダッシュボード: カテゴリ別・月別グラフ

---

## CLI ツール（bakuraku）コマンド設計

```bash
# セットアップ（Phase 1 を一括実行）
bakuraku setup [--base-token TOKEN]

# 経費精算（expense コマンドに統一）
bakuraku expense list    [--month 2026-04] [--status 申請中]
bakuraku expense create  --amount 3500 --category 交通費 --purpose "東京-大阪"
bakuraku expense approve <record-id>   # Approval tasks.approve を呼び出し
bakuraku expense reject  <record-id> --reason "理由"
bakuraku expense status  <record-id>   # Approval インスタンスの現在ステータス

# 稟議・申請（汎用 approval コマンド）
bakuraku approval list                 # 自分の承認待ちタスク一覧
bakuraku approval approve <instance-code>
bakuraku approval reject  <instance-code> --reason "理由"

# レポート
bakuraku report monthly [--month 2026-04] [--post-to <chat-id>]

# 認証
bakuraku auth            # lark-cli 認証状態確認
bakuraku auth login      # ユーザー OAuth 認証（user_access_token 取得）
```
...
### 3. bakuraku setup コマンド実装

**コマンド構造**:
```bash
bakuraku setup [--force] [--skip-form] [--skip-dashboard]
```
